...
export default function Siltarae() {
  ...
  return (
    <div className="min-h-screen p-8 flex flex-col items-center justify-center relative overflow-hidden" style={{ backgroundColor: '#F0F4FF' }}>
       ...
       <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-blue-400/10 blur-[120px] rounded-full"></div>

      <div className="max-w-2xl w-full relative z-10">
        <Link to="/activity" className="inline-flex items-center gap-2 text-blue-600 font-bold mb-8 hover:-translate-x-1 transition-transform">
          ...
          {!isSent ? (
            <div className="space-y-6">
              <textarea
                ...
                className="w-full h-48 p-6 rounded-3xl bg-white border-2 border-blue-50 focus:border-blue-400 outline-none transition-all resize-none shadow-inner"
              />
              <button
                onClick={handleSend}
                disabled={!worry.trim()}
                className="w-full py-5 bg-blue-500 text-white rounded-2xl font-black text-lg shadow-lg shadow-blue-500/30 hover:bg-blue-600 hover:-translate-y-1 transition-all disabled:opacity-50 disabled:translate-y-0 disabled:shadow-none"
              >
                걱정 날려보내기
              </button>
            </div>
            ...
            <div className="py-20 text-center animate-in fade-in zoom-in duration-700">
               <div className="relative inline-block">
                  <div className="absolute inset-0 bg-blue-400/20 blur-xl rounded-full scale-150"></div>
                  <Send className="w-24 h-24 text-blue-500 relative animate-bounce" />
               </div>
               ...
               <button 
                  ...
                  className="mt-10 px-8 py-3 bg-white border-2 border-blue-100 text-blue-500 rounded-full font-bold hover:bg-blue-50 transition-all"
               >
                  다른 걱정 풀기
               </button>
            </div>
