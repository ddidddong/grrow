...
export default function SweetSong() {
  ...
  return (
    <div className="min-h-screen p-8 flex flex-col items-center justify-center relative overflow-hidden" style={{ backgroundColor: '#F3FBF7' }}>
       ...
       <div className={`absolute inset-0 transition-all duration-[4000ms] ${breathingPhase === 'in' ? 'bg-[#6B9F7F]/10' : breathingPhase === 'hold' ? 'bg-[#B4D4C3]/10' : 'bg-[#F9F3E8]/50'}`}></div>
      ...
      <div className="max-w-2xl w-full relative z-10">
        ...
        <div className="bg-white/60 backdrop-blur-2xl rounded-[4rem] p-16 shadow-2xl border border-white/40 text-center">
          <div className="mb-12">
            <h1 className="text-4xl font-black mb-4 text-[#3D4F3D] tracking-tight">평온의 골골송</h1>
            <p className="text-[#6B9F7F] font-bold">따뜻한 고양이가 곁에서 골골대는 것처럼 편안한 숨을 쉬어봐요.</p>
          </div>

          <div className="relative flex items-center justify-center mb-16 h-64">
             {/* Breathing Ring */}
             <div 
               className={`absolute rounded-full border-4 border-[#6B9F7F]/20 transition-all duration-[4000ms] ease-in-out
                 ${breathingPhase === 'in' ? 'w-64 h-64 opacity-100' : breathingPhase === 'hold' ? 'w-64 h-64 opacity-80 scale-105' : 'w-32 h-32 opacity-40'}`}
             ></div>
             <div 
               className={`absolute rounded-full bg-gradient-to-br from-[#6B9F7F] to-[#B4D4C3] shadow-2xl transition-all duration-[4000ms] ease-in-out
          ...
          <div className="mb-16">
             <h2 className="text-2xl font-bold text-[#3D4F3D] mb-2 uppercase tracking-widest">
                {breathingPhase === 'in' ? '숨을 들이마셔요' : breathingPhase === 'hold' ? '잠시 멈춰요' : '천천히 내뱉어요'}
             </h2>
             <p className="text-[#6B9F7F] font-black">코로 깊게 들이마시고, 입으로 얇고 길게 내보내세요.</p>
          </div>

          <button
            onClick={() => setIsActive(!isActive)}
            className={`px-12 py-6 rounded-3xl font-black text-xl shadow-xl transition-all active:scale-95
              ${isActive ? 'bg-white text-[#6B9F7F] border-2 border-[#6B9F7F]/10' : 'bg-[#6B9F7F] text-white hover:bg-[#5A8D6F] shadow-[#6B9F7F]/20'}`}
          >
          ...
          <div className="mt-12 flex justify-center gap-8">
             <div className="flex flex-col items-center gap-2">
                <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-sm">
                   <Music className="w-6 h-6 text-[#6B9F7F]" />
                </div>
                <span className="text-[10px] font-bold text-[#6B9F7F]">명상 음악</span>
             </div>
             <div className="flex flex-col items-center gap-2">
                <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-sm">
                   <Wind className="w-6 h-6 text-[#6B9F7F]" />
                </div>
                <span className="text-[10px] font-bold text-[#6B9F7F]">환경음</span>
             </div>
          </div>
        </div>
        
        <div className="mt-10 bg-[#6B9F7F]/10 backdrop-blur-md rounded-3xl p-6 border border-[#6B9F7F]/10 flex items-center gap-5">
           <Sparkles className="w-8 h-8 text-[#6B9F7F]" />
           <p className="text-sm text-[#3D4F3D] leading-relaxed font-medium">
